...
def print_system_status():
    ...
    print(f"Vector database: {'Ready' if system_info['vector_db_exists'] else 'Missing'}")
    print(f"Retriever cache: {'Ready' if system_info['retriever_cached'] else 'Will be created'}")
...
def select_provider(available_providers):
    ...
    for i, provider in enumerate(available_providers, 1):
        ...
        print(f"     Speed: {info.get('speed', 'N/A')} | Quality: {info.get('quality', 'N/A')}")
    
    while True:
        try:
            choice = input(f"\nChoose provider (1-{len(available_providers)}): ").strip()
            idx = int(choice) - 1
...
def show_chat_history():
    ...
    for i, interaction in enumerate(recent_history, 1):
        ...
        print(f"{i}. [{timestamp}] [{interaction['provider']}] ({interaction['performance']['total_time']}s)")
        print(f"   Q: {interaction['question'][:70]}...")
        if len(interaction['question']) > 70:
            print("      " + interaction['question'][70:140] + ("..." if len(interaction['question']) > 140 else ""))
        print()
...
def interactive_chat():
    ...
    while True:
        ...
        if query.lower() == 'clear':
            confirm = input("Are you sure you want to clear chat history? (y/N): ").strip().lower()
            if confirm == 'y':
        ...
        try:
            ...
            print(answer)
            print("-" * 60)
            print(f"{stats['total_time']:.1f}s | {stats['pages_retrieved']} pages | {stats['provider']}")
...
def test_both_providers():
    ...
    for provider in available_providers:
        ...
        try:
            ...
            print(f"Retrieved {stats['documents_retrieved']} docs from {stats['pages_retrieved']} pages")
