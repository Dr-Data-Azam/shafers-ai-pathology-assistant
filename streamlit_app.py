...
from chat_history_manager import load_chat_history, clear_chat_history, get_history_stats
...
def check_system_status():
    ...
    if not system_info["vector_db_exists"]:
        st.error("Vector database not found! Please run `python vector_store_creator.py` first.")
        return False
    
    if not system_info["available_providers"]:
        st.error("No API keys found! Please set OPENAI_API_KEY or GROQ_API_KEY in your .env file.")
        return False
...
def render_main_chat():
    ...
    st.markdown('<h1 class="main-header">Shafer\'s Oral Pathology Q&A</h1>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        for provider in available_providers:
            ...
            provider_options[f"{provider.upper()} ({info.get('speed', 'N/A')} speed)"] = provider
    ...
    if selected_sample != "Select a sample question..." and selected_sample != st.session_state.get('current_question', ''):
    ...
    if submit_button and question.strip():
        with st.spinner(f"🤔 Processing with {st.session_state.current_provider.upper()}..."):
            try:
                start_time = time.time()
                answer, stats = ask_question(question, st.session_state.current_provider)
                end_time = time.time()
                ...
                st.info("📚 Answer based on Shafer's Textbook of Oral Pathology (7th Edition)")
...
def render_chat_history():
    ...
    for i, chat in enumerate(reversed(st.session_state.chat_history[-5:])):  # Show last 5
        with st.expander(f"{chat['timestamp'].strftime('%H:%M:%S')} - {chat['question'][:60]}..."):
...
def render_persistent_history():
    ...
    for entry in history:
        df_data.append({
            ...
            'Question': entry['question'][:70] + "..." if len(entry['question']) > 70 else entry['question'],
    ...
    if st.session_state.get('show_analytics', False):
        ...
        if len(history) > 1:
            # Response time chart
            response_times = [entry['performance']['total_time'] for entry in history[-20:]]
            timestamps = [datetime.fromisoformat(entry['timestamp']).strftime('%H:%M') for entry in history[-20:]]
